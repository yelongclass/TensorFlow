...
#url = 'http://blog.topspeedsnail.com/wp-content/uploads/2016/12/trainPassigers.csv'
#ass_data = requests.get(url).content

#df = pd.read_csv(io.StringIO(ass_data.decode('utf-8')))  # python2 uses StringIO.StringIO
df = pd.read_csv('data/trainPassigers.csv')

data = np.array(df['Railway passenger volume _ current value (million) '])
# normalize
...
def train_rnn():
	...
	with tf.Session() as sess:
		...
		for step in range(10000):
			...
			if step % 10 == 0:
				# Use testData to calculate loss
				print(step, loss_)
